...
describe('Direct API tests', function () {
    ...
    describe('Grant entitlements', function () {
        it('grant entitlement', async () => {
            ...
            // when
            const response = await (0, apiV3Utils_1.executeGrantEntitlementsRequest)(network_1.API_URL, constants_1.PRIVATE_TOKEN_FOR_TESTS, userId, entitlementId, expires);
        ...
        it('grant entitlement twice', async () => {
            ...
            await (0, apiV3Utils_1.executeGrantEntitlementsRequest)(network_1.API_URL, constants_1.PRIVATE_TOKEN_FOR_TESTS, userId, entitlementId, expiresOld);
            ...
            const requestStartTs = (0, utils_1.getCurrentTs)();
            // when
            const response = await (0, apiV3Utils_1.executeGrantEntitlementsRequest)(network_1.API_URL, constants_1.PRIVATE_TOKEN_FOR_TESTS, userId, entitlementId, expiresNew);
            const requestEndTs = (0, utils_1.getCurrentTs)();
            const responseBody = await response.json();
            // then
            expect(response.status).toBe(200);
        ...
        it('grant entitlement with wrong token', async () => {
            ...
            // when
            const response = await (0, apiV3Utils_1.executeGrantEntitlementsRequest)(network_1.API_URL, constants_1.PROJECT_KEY_FOR_TESTS, userId, entitlementId, expires);
            const responseBody = await response.json();
            // then
    ...
    describe('Revoke entitlements', function () {
        it('revoke existing entitlement', async () => {
            ...
            const entitlementId = 'Test Permission';
            const expires = (0, utils_1.getCurrentTs)() + 10000;
            await (0, apiV3Utils_1.executeGrantEntitlementsRequest)(network_1.API_URL, constants_1.PRIVATE_TOKEN_FOR_TESTS, userId, entitlementId, expires);
            // when
            const response = await (0, apiV3Utils_1.executeRevokeEntitlementsRequest)(network_1.API_URL, constants_1.PRIVATE_TOKEN_FOR_TESTS, userId, entitlementId);
            // then
            expect(response.status).toBe(200);
        });
        ...
        it('revoke entitlement with wrong token', async () => {
            ...
            // when
            const response = await (0, apiV3Utils_1.executeRevokeEntitlementsRequest)(network_1.API_URL, constants_1.PROJECT_KEY_FOR_TESTS, userId, entitlementId);
            const responseBody = await response.json();
            // then
            expect(response.status).toBe(401);
            expect(responseBody.error).toEqual(expError);
